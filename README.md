...
qtip runs on cpu and automatically uses a cuda gpu when ClearVoice detects one. the model is downloaded to the qtip cache on first use.

## usage

```sh
qtip input.mov output.mp4
```
...
```sh
qtip --lufs -12 input.mov output.mp4
```
